...
class WooCommerce_Downloads {
    ...
    public function get_redirect_url_for_entry(CacheNode $cached_entry) {
        ...
        if (empty($direct_download_url)) {
            ...
            $redirect_link = 'https://www.googleapis.com/drive/v3/files/' . $cached_entry->get_id() . '/export?mimeType=' . urlencode($mimetype) . '&alt=media&userIp=' . Helpers::get_user_ip() . "&access_token=" . $token->access_token;
        } else {
            $redirect_link = $direct_download_url . '&access_token=' . $token->access_token . '&userIp=' . Helpers::get_user_ip();
            $redirect_link = 'https://www.googleapis.com/drive/v3/files/' . $cached_entry->get_id() . '?alt=media&userIp=' . Helpers::get_user_ip() . "&access_token=" . $token->access_token;
        }
