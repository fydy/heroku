	...
	<div id="layout-tab" class="tab-product">
		...
		<div class="tab-bd dom-display">
			...
			<ul class="tab-bd-con current">
				<?php query_posts('showposts='.zm_get_option('tabt_n').'&category__and='.zm_get_option('tabt_id')); while (have_posts()) : the_post(); ?>
			...
			<ul class="tab-bd-con">
				<?php query_posts('showposts='.zm_get_option('tabt_n').'&category__and='.zm_get_option('tabz_n')); while (have_posts()) : the_post(); ?>
				<?php the_title( sprintf( '<li class="list-title"><a href="%s" rel="bookmark"><i class="be be-arrowright"></i>', esc_url( get_permalink() ) ), '</a></li>' ); ?>
				<?php endwhile; ?>
				<?php wp_reset_query(); ?>
		    </ul>
			...
			<ul class="tab-bd-con">
				<?php query_posts('showposts='.zm_get_option('tabt_n').'&category__and='.zm_get_option('tabq_n')); while (have_posts()) : the_post(); ?>
				<?php the_title( sprintf( '<li class="list-title"><a href="%s" rel="bookmark"><i class="be be-arrowright"></i>', esc_url( get_permalink() ) ), '</a></li>' ); ?>
				<?php endwhile; ?>
				<?php wp_reset_query(); ?>
			</ul>
