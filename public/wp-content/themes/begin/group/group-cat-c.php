...
<div class="g-row <?php if (zm_get_option('bg_18')) { ?>g-line<?php } ?> sort" name="<?php echo zm_get_option('group_cat_c_s'); ?>">
	<div class="g-col">
		<div class="group-cat">
			...
			<div class="gr2">
				<div class="gr-cat-box">
					...
					<div class="gr-cat-more wow fadeInDown" data-wow-delay="0.7s"><a href="<?php echo get_category_link($category);?>" title="<?php echo strip_tags(category_description($category)); ?>"><?php _e( '更多', 'begin' ); ?> <i class="be be-fastforward"></i></a></div>
					<div class="clear"></div>
					<div class="gr-cat-site">
						<?php if (zm_get_option('group_cat_c_top')) { ?>
							...
							<?php query_posts( array ( 'category__in' => array(get_query_var('cat')), 'meta_key' => 'cat_top', 'showposts' => 1, 'ignore_sticky_posts' => 1 ) ); while ( have_posts() ) : the_post(); $do_not_cat[] = $post->ID;?>
								<figure class="gr-thumbnail wow fadeInUp" data-wow-delay="0.3s"><?php if (zm_get_option('lazy_s')) { zm_long_thumbnail_h(); } else { zm_long_thumbnail(); } ?></figure>
