			...
			<?php if ( get_option('comment_registration') && !$user_ID ) : ?>
				<p class="comment-nologin">
					<?php if (zm_get_option('user_l')) { ?>
						<?php print '' . sprintf(__( '您必须', 'begin' )) . ''; ?><a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php echo urlencode(get_permalink()); ?>"><?php _e( '登录', 'begin' ); ?></a><?php _e( '才能发表评论！', 'begin' ); ?>
					<?php } else { ?>
						<?php print '' . sprintf(__( '您必须', 'begin' )) . ''; ?><a href="#login" id="login-c" ><span class="login-respond"><?php _e( '登录', 'begin' ); ?></span></a><?php _e( '才能发表评论！', 'begin' ); ?>
					<?php } ?>
				</p>
			<?php else : ?>
