...
function create_show_h_a_meta_box() {
	...
	if (function_exists('add_meta_box')) {
		add_meta_box('create_show_h_a_meta_box', '产品幻灯设置-1', 'new_meta_show_h_a_boxes', 'show', 'normal', 'high');
	}
}
...
// 面板内容
function new_meta_show_h_b_boxes() {
	global $post, $new_meta_show_h_b_boxes;
	//获取保存
	foreach ($new_meta_show_h_b_boxes as $meta_box) {
		$meta_box_value = get_post_meta($post->ID, $meta_box['name'] . '', true);
		if ($meta_box_value != "")
		//将默认值替换为已保存的值
		$meta_box['std'] = $meta_box_value;
...
function create_show_h_b_meta_box() {
	global $theme_name;
	if (function_exists('add_meta_box')) {
		add_meta_box('create_show_h_b_meta_box', '产品幻灯设置-2', 'new_meta_show_h_b_boxes', 'show', 'normal', 'high');
	}
}
function save_show_h_b_postdata($post_id) {
	global $post, $new_meta_show_h_b_boxes;
	foreach ($new_meta_show_h_b_boxes as $meta_box) {
		if (!wp_verify_nonce($_POST[$meta_box['name'] . '_noncename'], plugin_basename(__FILE__))) {
			return $post_id;
...
add_action('admin_menu', 'create_show_h_b_meta_box');
add_action('save_post', 'save_show_h_b_postdata');
